...
function BattleNotify(mod) {
  ...
  function Conditions() {
    function AbnormalConditions() {
      ...
      function checkAddedOrRefreshed(
        lastMatch,
        { stacks = 0, added, refreshed } = {}
      ) {
        if (stacks > this.requiredStacks) return refreshed || added;
      }
    ...
    function CooldownConditions() {
      ...
      function checkExpiringDuringCombat(lastMatch, { expires = 0 } = {}) {
        if (combat()) return checkExpiring.call(this, ...arguments);
      }
      ...
      function checkExpiringDuringEnrage(lastMatch, { expires = 0 } = {}) {
        if (enrage) return checkExpiringDuringCombat.call(this, ...arguments);
      }
      ...
      function checkReadyDuringCombat(lastMatch, { expires = 0 } = {}) {
        if (combat()) return checkReady.call(this, ...arguments);
      }
      ...
      function checkReadyDuringEnrage(lastMatch, { expires = 0 } = {}) {
        if (enrage()) return checkReadyDuringCombat.call(this, ...arguments);
      }
  ...
  function Targets() {
    function AbnormalTargets() {
      ...
      this.party = () =>
        party.members().filter((cid) => cid !== entities.myCid());
  ...
  function checkAbnormalEvent(entityId, event) {
    ...
    if (!event.lastMatches.has(entityId)) event.lastMatches.set(entityId, 0);
  ...
  function CooldownEvent(data) {
    ...
    this.check = function () {
      getTargets()
        .map((info) => tryIt(() => checkCooldownEvent(info, event)))
        .filter(isError)
        .forEach((err) =>
          logError([
  ...
  function checkCooldownEvent(info, event) {
    ...
    if (!event.lastMatches.has(id)) event.lastMatches.set(id, 0);
  ...
  function loadEvent(event) {
    let type;
    if (event.abnormalities) type = AbnormalEvent;
    else if (event.type && event.type.toLowerCase() === 'reset')
      type = ResetEvent;
    else if (event.skills || event.items) type = CooldownEvent;
