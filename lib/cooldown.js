...
module.exports = function CooldownManager(mod) {
  ...
  const skillIcon = (id) =>
    `<img src='img://skill__0__${model}__${id}' width='48' height='48' vspace='-7' />`;
  const itemIcon = (id) =>
    `<img src='img://item__${id}' width='48' height='48' vspace='-7' />`;
  ...
  this.onReset = function (skills, cb) {
    skills
      ...
      .forEach(({ group, info }) => getHooks(group).push(cb.bind(null, info)));
  };
  this.skill = getSkill;
  this.item = getItem;
};
